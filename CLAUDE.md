...
Named kuji-in terms in the English text follow a fixed romanization + gloss convention (e.g. `Tokko-in ("Vajra Seal")`, `Ongyo-in ("Concealment Seal")`) — reuse the existing romanizations rather than inventing new spellings if a term recurs. No macrons (ASCII romaji only, matching common English usage for these terms).
